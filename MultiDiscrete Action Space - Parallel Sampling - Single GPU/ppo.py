...
def train(envs, name, num_envs, action_space, target_kl, minibatch_size, gamma, ent_coef, vf_coef, num_nn, critic_std, actor_std, learning_rate, num_epoch_steps):
    ...
    agent = Agent(envs, action_space, num_nn, critic_std, actor_std).to(device)
    ...
    obs = torch.zeros((num_env_steps, num_envs) + envs.single_observation_space.shape).to(device)
    actions = torch.zeros((num_env_steps, num_envs, len(action_space))).to(device)
    logprobs = torch.zeros((num_env_steps, num_envs)).to(device)
    rewards = torch.zeros((num_env_steps, num_envs)).to(device)
    dones = torch.zeros((num_env_steps, num_envs)).to(device)
    values = torch.zeros((num_env_steps, num_envs)).to(device)
    ...
    for round in range(num_rounds):
        ...
        for step in range(num_env_steps):
            ...
            with torch.no_grad():
                action, logprob, _, value = agent.get_action_and_value(next_obs.reshape((num_envs, -1)))
