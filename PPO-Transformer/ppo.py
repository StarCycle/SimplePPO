...
from TransformerForRL import TransformerForRL
...
class Agent(nn.Module):
    def __init__(self, envs, action_space, num_nn, critic_std, actor_std):
        ...
        self.transformer = TransformerForRL(state_dim, num_nn, 2, 32, 16, 1, 0.1)
        self.critic = layer_init(nn.Linear(num_nn, 1), std=critic_std)
        self.actor = layer_init(nn.Linear(num_nn, sum(action_space)), std=actor_std)

    def get_value(self, x):
        return self.critic(self.transformer(x))

    def get_action_and_value(self, x, action=None):
        logits = self.actor(self.transformer(x))
        ...
        return action, logprob.sum(0), entropy.sum(0), self.critic(self.transformer(x))

def train(env, name, action_space, target_kl, minibatch_size, gamma, ent_coef, vf_coef, num_nn, critic_std, actor_std, learning_rate, num_epoch_steps):
    ...
    optimizer = optim.Adam(agent.parameters(), lr=learning_rate, weight_decay=1e-4, eps=1e-5)
                            ...
                            for cstd in critic_std:
                                for astd in actor_std:
                                    for lr in learning_rate:
                                        ...
                                        train(env, name, action_space, tk, bs, ga, ef, vf, num, cstd, astd, lr, num_epoch_steps)
